        ...
        if [ -n "$RUN_ID" ]; then
            ...
            if [ -n "$JOB_ID" ] && [ "$JOB_ID" != "null" ]; then
                ...
                if echo "$LOGS" | grep -q "XKEYBOARD"; then
                    ...
                    echo "Update your Xvfb configuration in the workflow file:"
                    echo "  Xvfb :99 -screen 0 1280x1024x24 -nolisten tcp -ac +extension RANDR +render -noreset -noxkbmap &"
                    echo "  export DISPLAY=:99"
                    echo ""
                    echo "Or use the xvfb-run with the -noxkbmap flag:"
                    echo "  xvfb-run --auto-servernum --server-args=\"-screen 0 1280x1024x24 -noxkbmap\" drawio -x -f svg -o ..."
                else
...
echo "   • Add the -noxkbmap flag to Xvfb configuration"
echo ""
